...
for bus in busses:
	devices = bus.devices
	for dev in devices:
		if dev.idVendor==0xa5a5 and dev.idProduct==0x01:
		   print "To jest nasze urzadzenie!"
		   mydev=dev
...
f=mydev.open()
f.setConfiguration(mydev.configurations[1])
f.claimInterface(0)
f.bulkWrite(1,"\000")
...
adc_cfg += chr(0)+chr(1)+chr(2)+chr(33)+chr(4)+chr(5)+chr(6)+chr(7)+chr(0)
...
while True:
   s=f.bulkRead(2,3000)
   p+=1
   print [hex(i) for i in s[0:len(s)]]
   #prstr(s)
